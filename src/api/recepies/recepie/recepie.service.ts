...
export class RecepieService {
  ...
  constructor(@InjectRepository(Recepie) private readonly recepieRepository: Repository<Recepie>,
  ) {
  ...
  async findOne(id: string) {
    const recepie = await this.recepieRepository.findOne(id).catch(() => {
      throw new InternalServerErrorException(`Failed to find recepie id: ${id} !`)
    });
    return recepie
  }
  ...
  async update(id: string, updateRecepieDto: UpdateRecepieDto) {
    ...
    entity.id = id
    const recepie = await this.recepieRepository.save(updateRecepieDto).catch(() => {
      throw new InternalServerErrorException(`Failed to update recepie id: ${id} !`)
    });
    return recepie
  }
  ...
  async remove(id: string) {
    await this.recepieRepository.delete(id).catch(() => {
      throw new InternalServerErrorException(`Failed to delete recepie id: ${id} !`)
    });
